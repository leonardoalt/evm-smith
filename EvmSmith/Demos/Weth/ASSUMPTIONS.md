...
## (W) Weth-specific — new shape, please flag if any seem off

### W1. **Storage layout pinned**: `tokenSum σ C := Σ_a storageOf σ C (addressSlot a)` ☐
> Weth's user balances live at storage slots `addressSlot a` for
> `a : AccountAddress`, where `addressSlot a` zero-extends the 20-byte
> address to a 32-byte UInt256.

This is a *definition*, not an assumption — but it's a *Weth-specific*
choice (not all token contracts use this layout; most use
`keccak256(addr . 0)` for `mapping(address => uint256)`). The
informal proof's ΔS calculations all assume this layout. If the
on-chain Weth uses a different slot scheme, our `tokenSum` definition
needs adjusting. Solidity devs writing Weth-by-hand (as we did) often
pick this simpler layout because it's gas-cheaper and avoids the
keccak preimage. Flagging here for explicit acceptance of *this
specific* layout.

### W2. **Storage at C is only modifiable from frames at codeOwner = C** ☐
> Provable from EVM SSTORE semantics. Lemma needed:
> `step_modifies_storage_only_at_codeOwner`.

Not an assumption per se — *should be a theorem*. Listed here so
you're aware we'll need to prove it. ~30 LoC; can live in
`StepFrame.lean`.

### W3. **Balance changes at C come from a closed channel set** ☐
> Provable. The set is: Θ value transfer (in or out), SELFDESTRUCT
> with beneficiary = C, CREATE/CREATE2 with beneficiary = C
> (excluded by T5), coinbase reward (excluded by B2), tx-level fee
> credit (excluded by B1).

Theorem, not assumption. Sketch already in
`MutualFrame`-existing material.

### W4. **No reentrant withdraw can drain Weth** — *derivable from
the bytecode*, ☐
> Provable: the SSTORE at PC 60 decrements `storage[CALLER]` *before*
> the CALL at PC 72. Reentrant calls see the decremented balance
> and either revert or recurse on a smaller balance, terminating by
> fuel induction.

Theorem, derived inside the bytecode walk. Not an assumption.

### W5. **`addressSlot` is an injection** ☐
> Different addresses map to different storage slots: `a ≠ b →
> addressSlot a ≠ addressSlot b`.

Trivially true for the zero-extend embedding (a 20-byte injective
into 32 bytes). `decide`-discharged. Listed here to flag that
`tokenSum`'s correctness depends on it.
...
| Framework-gap (F) | F1, F2 | Accept as hypotheses for now (derivable when Phase A.2 lands) |
| Weth-specific (W) | W1 (definition), W2/W3/W4/W5 (theorems, not assumptions) | Confirm storage layout (W1) |

If any item should be reframed as a theorem (we should prove it
rather than assume) or rejected outright, say so.

The genuinely *new* trust ask compared to Register is **W1
(storage layout)**. Everything else is structurally identical to
what Register already accepts.
