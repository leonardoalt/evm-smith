...
/// @title Per-tx gas comparison: bytecode-original vs PUSH0-optimized vs Solidity
...
///         Bytecode-orig vs bytecode-opt: difference is purely the
///         PUSH0-vs-PUSH1-zero schedule (1 gas per swap site
///         traversed). Bytecode-orig vs Solidity: the Solidity bill
///         is dominated by compiler-emitted dispatcher overhead,
///         calldata length checks, function-selector matching, the
///         keccak-derived mapping-slot lookup, etc.
contract WethGasCompareTest is Test {
    ...
    /// Deposit on all three. Bytecode-orig vs bytecode-opt: 1 gas
    /// saving (one PUSH0 site reached). Bytecode vs Solidity: the
    /// Solidity dispatcher + slot lookup adds nontrivial overhead.
    ...
    function test_gas_withdraw_success() public {
        // Pre-fund all three so the withdraw can pay out.
        ...
        assertTrue(okO, "orig withdraw success");
        assertTrue(okP, "opt withdraw success");
        assertTrue(okS, "sol withdraw success");
        ...
        assertLe(gOpt, gOrig, "optimized withdraw-success must not cost more gas");
    }
    ...
    /// Unknown selector on all three. The two bytecode versions revert
    /// via their explicit no-selector-match path; the Solidity contract
