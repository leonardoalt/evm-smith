...
This is the **engine** behind the auditor-facing `Spec.lean`; read
`Spec.lean`, not this file, for *what* WETH guarantees. This file holds
*how* the behaviour is proved from the bytecode:

* the behavioural vocabulary used by the postconditions
  (`recordedBalance`, `msgSender`, `msgValue`, `withdrawArg`,
  `tokenBalanceSlot`, `functionSelector`);
* the storage-readback lemmas (`recordedBalance_*`);
* the bytecode-walking entry-point theorems — dispatch routing
...
`Spec.lean` re-exposes the headline behavioural results
(`weth_deposit_run`, `weth_unknown_run`, `weth_withdraw_run`) as
one-line restatements of the `*_run_impl` theorems below.
-/
...
Readable wrappers over the EVMYulLean state projections, used by the
postconditions of the entry-point theorems below. -/
...
theorem weth_withdraw_to_call_impl
    (s0 : EVM.State) (callFuel N : ℕ) (s_pre : EVM.State)
    (C : Address) (acc : Account .EVM)
    (hcode : s0.executionEnv.code = wethBytecode)
    (hpc0 : s0.pc = UInt256.ofNat 0)
    (hstk0 : s0.stack = [])
    ...
    (hle : (withdrawArg s0).toNat
            ≤ (acc.lookupStorage (tokenBalanceSlot s0.executionEnv.source)).toNat)
    (hrun : evmRunToCall callFuel N s0 = some s_pre) :
    recordedBalance s_pre.accountMap C (msgSender s0)
        = UInt256.sub (recordedBalance s0.accountMap C (msgSender s0)) (withdrawArg s0) := by
  have hcodeB : s0.executionEnv.code = bytecode := hcode
  have hselB : selectorOf s0.executionEnv.calldata = withdrawSelector := hsel
  have hne_depB : selectorOf s0.executionEnv.calldata ≠ depositSelector := by
  ...
  have hd0 : decode s0.executionEnv.code s0.pc = some (.Push .PUSH1, some (UInt256.ofNat 0, 1)) := by
    rw [hcodeB, hpc0]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s1, hst0, hrun⟩ := evmRunToCall_succ_step callFuel N s0 _ _ _ hd0 (by decide) (by decide) hrun
  have h0 : EVM.step (callFuel + 1) 0 (decode s0.executionEnv.code s0.pc) s0 = .ok s1 := by
    rw [hd0]; exact hst0
  obtain ⟨h1pc, h1stk, h1ee, _⟩ := step_PUSH1_shape_strong s0 s1 callFuel 0 (UInt256.ofNat 0) hst0
  rw [hstk0] at h1stk
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s2, hst1, hrun⟩ := evmRunToCall_succ_step callFuel N s1 _ _ _ hd1 (by decide) (by decide) hrun
  have h1 : EVM.step (callFuel + 1) 0 (decode s1.executionEnv.code s1.pc) s1 = .ok s2 := by
    rw [hd1]; exact hst1
  obtain ⟨h2pc, _, h2ee, _⟩ := step_CALLDATALOAD_value s1 s2 callFuel 0 none (UInt256.ofNat 0) [] h1stk hst1
  have hc2 : s2.executionEnv.code = bytecode := by rw [h2ee]; exact hc1
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s3, hst2, hrun⟩ := evmRunToCall_succ_step callFuel N s2 _ _ _ hd2 (by decide) (by decide) hrun
  have h2 : EVM.step (callFuel + 1) 0 (decode s2.executionEnv.code s2.pc) s2 = .ok s3 := by
    rw [hd2]; exact hst2
  obtain ⟨h3pc, _, h3ee, _⟩ := step_PUSH1_shape_strong s2 s3 callFuel 0 (UInt256.ofNat 0xe0) hst2
  have hc3 : s3.executionEnv.code = bytecode := by rw [h3ee]; exact hc2
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s4, hst3, hrun⟩ := evmRunToCall_succ_step callFuel N s3 _ _ _ hd3 (by decide) (by decide) hrun
  have h3 : EVM.step (callFuel + 1) 0 (decode s3.executionEnv.code s3.pc) s3 = .ok s4 := by
    rw [hd3]; exact hst3
  obtain ⟨hs4stk, hs4pc, hs4ee, _⟩ :=
    weth_dispatcher_computes_selector s0 s1 s2 s3 s4 callFuel 0 0 0 0 hcodeB hpc0 hstk0 h0 h1 h2 h3
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s5, hst4, hrun⟩ := evmRunToCall_succ_step callFuel N s4 _ _ _ hd4 (by decide) (by decide) hrun
  have h4 : EVM.step (callFuel + 1) 0 (decode s4.executionEnv.code s4.pc) s4 = .ok s5 := by
    rw [hd4]; exact hst4
  obtain ⟨h5pc, h5stk, h5ee, _⟩ := step_DUP1_shape_strong s4 s5 callFuel 0 none withdrawSelector [] hs4stk hst4
  rw [hs4stk] at h5stk
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s6, hst5, hrun⟩ := evmRunToCall_succ_step callFuel N s5 _ _ _ hd5 (by decide) (by decide) hrun
  have h5 : EVM.step (callFuel + 1) 0 (decode s5.executionEnv.code s5.pc) s5 = .ok s6 := by
    rw [hd5]; exact hst5
  obtain ⟨h6pc, h6stk, h6ee, _⟩ :=
    step_PUSH_shape_strong s5 s6 callFuel 0 .PUSH4 (by decide) depositSelector 4 hst5
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s7, hst6, hrun⟩ := evmRunToCall_succ_step callFuel N s6 _ _ _ hd6 (by decide) (by decide) hrun
  have h6 : EVM.step (callFuel + 1) 0 (decode s6.executionEnv.code s6.pc) s6 = .ok s7 := by
    rw [hd6]; exact hst6
  obtain ⟨h7pc, h7stk, h7ee, _⟩ :=
    step_EQ_value s6 s7 callFuel 0 none depositSelector withdrawSelector [withdrawSelector] h6stk hst6
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s8, hst7, hrun⟩ := evmRunToCall_succ_step callFuel N s7 _ _ _ hd7 (by decide) (by decide) hrun
  have h7 : EVM.step (callFuel + 1) 0 (decode s7.executionEnv.code s7.pc) s7 = .ok s8 := by
    rw [hd7]; exact hst7
  obtain ⟨h8pc, h8stk, h8ee, _⟩ :=
    step_PUSH_shape_strong s7 s8 callFuel 0 .PUSH2 (by decide) depositLbl 2 hst7
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s9, hst8, hrun⟩ := evmRunToCall_succ_step callFuel N s8 _ _ _ hd8 (by decide) (by decide) hrun
  have h8 : EVM.step (callFuel + 1) 0 (decode s8.executionEnv.code s8.pc) s8 = .ok s9 := by
    rw [hd8]; exact hst8
  obtain ⟨h9pc, h9stk, h9ee, _⟩ :=
    step_JUMPI_shape_strong s8 s9 callFuel 0 none depositLbl
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s10, hst9, hrun⟩ := evmRunToCall_succ_step callFuel N s9 _ _ _ hd9 (by decide) (by decide) hrun
  have h9 : EVM.step (callFuel + 1) 0 (decode s9.executionEnv.code s9.pc) s9 = .ok s10 := by
    rw [hd9]; exact hst9
  obtain ⟨h10pc, h10stk, h10ee, _⟩ :=
    step_PUSH_shape_strong s9 s10 callFuel 0 .PUSH4 (by decide) withdrawSelector 4 hst9
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s11, hst10, hrun⟩ := evmRunToCall_succ_step callFuel N s10 _ _ _ hd10 (by decide) (by decide) hrun
  have h10 : EVM.step (callFuel + 1) 0 (decode s10.executionEnv.code s10.pc) s10 = .ok s11 := by
    rw [hd10]; exact hst10
  obtain ⟨h11pc, h11stk, h11ee, _⟩ :=
    step_EQ_value s10 s11 callFuel 0 none withdrawSelector withdrawSelector [] h10stk hst10
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s12, hst11, hrun⟩ := evmRunToCall_succ_step callFuel N s11 _ _ _ hd11 (by decide) (by decide) hrun
  have h11 : EVM.step (callFuel + 1) 0 (decode s11.executionEnv.code s11.pc) s11 = .ok s12 := by
    rw [hd11]; exact hst11
  obtain ⟨h12pc, _, h12ee, _⟩ :=
    step_PUSH_shape_strong s11 s12 callFuel 0 .PUSH2 (by decide) withdrawLbl 2 hst11
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s13, hst12, hrun⟩ := evmRunToCall_succ_step callFuel N s12 _ _ _ hd12 (by decide) (by decide) hrun
  have h12 : EVM.step (callFuel + 1) 0 (decode s12.executionEnv.code s12.pc) s12 = .ok s13 := by
    rw [hd12]; exact hst12
  obtain ⟨h13pc, h13stk, h13ee, h13am⟩ :=
    weth_routes_withdraw s0 s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 s11 s12 s13 callFuel
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s14, hst13, hrun⟩ := evmRunToCall_succ_step callFuel N s13 _ _ _ hd13d (by decide) (by decide) hrun
  have h13' : EVM.step (callFuel + 1) 0 (decode s13.executionEnv.code s13.pc) s13 = .ok s14 := by
    rw [hd13d]; exact hst13
  obtain ⟨h14pc, h14stk, h14ee, _⟩ := step_JUMPDEST_shape_strong s13 s14 callFuel 0 none hst13
  rw [h13stk] at h14stk
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s15, hst14, hrun⟩ := evmRunToCall_succ_step callFuel N s14 _ _ _ hd14d (by decide) (by decide) hrun
  have h14' : EVM.step (callFuel + 1) 0 (decode s14.executionEnv.code s14.pc) s14 = .ok s15 := by
    rw [hd14d]; exact hst14
  obtain ⟨h15pc, h15stk, h15ee, _⟩ := step_PUSH1_shape_strong s14 s15 callFuel 0 (UInt256.ofNat 4) hst14
  rw [h14stk] at h15stk
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s16, hst15, hrun⟩ := evmRunToCall_succ_step callFuel N s15 _ _ _ hd15d (by decide) (by decide) hrun
  have h15' : EVM.step (callFuel + 1) 0 (decode s15.executionEnv.code s15.pc) s15 = .ok s16 := by
    rw [hd15d]; exact hst15
  obtain ⟨h16pc, h16stk, h16ee, _⟩ :=
    step_CALLDATALOAD_value s15 s16 callFuel 0 none (UInt256.ofNat 4) [] h15stk hst15
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s17, hst16, hrun⟩ := evmRunToCall_succ_step callFuel N s16 _ _ _ hd16d (by decide) (by decide) hrun
  have h16'' : EVM.step (callFuel + 1) 0 (decode s16.executionEnv.code s16.pc) s16 = .ok s17 := by
    rw [hd16d]; exact hst16
  obtain ⟨h17pc, h17stk, h17ee, _⟩ := step_CALLER_value s16 s17 callFuel 0 none hst16
  rw [h16stk] at h17stk
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s18, hst17, hrun⟩ := evmRunToCall_succ_step callFuel N s17 _ _ _ hd17d (by decide) (by decide) hrun
  have h17' : EVM.step (callFuel + 1) 0 (decode s17.executionEnv.code s17.pc) s17 = .ok s18 := by
    rw [hd17d]; exact hst17
  obtain ⟨h18pc, h18stk, h18ee, _⟩ :=
    step_DUP1_shape_strong s17 s18 callFuel 0 none (UInt256.ofNat s16.executionEnv.source.val) [xV] h17stk hst17
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s19, hst18, hrun⟩ := evmRunToCall_succ_step callFuel N s18 _ _ _ hd18d (by decide) (by decide) hrun
  have h18' : EVM.step (callFuel + 1) 0 (decode s18.executionEnv.code s18.pc) s18 = .ok s19 := by
    rw [hd18d]; exact hst18
  obtain ⟨h19pc, h19stk, h19ee, _⟩ :=
    step_SLOAD_shape_strong s18 s19 callFuel 0 none (UInt256.ofNat s16.executionEnv.source.val)
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s20, hst19, hrun⟩ := evmRunToCall_succ_step callFuel N s19 _ _ _ hd19d (by decide) (by decide) hrun
  have h19' : EVM.step (callFuel + 1) 0 (decode s19.executionEnv.code s19.pc) s19 = .ok s20 := by
    rw [hd19d]; exact hst19
  obtain ⟨h20pc, h20stk, h20ee, _⟩ :=
    step_DUP3_shape_strong s19 s20 callFuel 0 none balV (UInt256.ofNat s16.executionEnv.source.val) xV [] h19stk hst19
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s21, hst20, hrun⟩ := evmRunToCall_succ_step callFuel N s20 _ _ _ hd20d (by decide) (by decide) hrun
  have h20' : EVM.step (callFuel + 1) 0 (decode s20.executionEnv.code s20.pc) s20 = .ok s21 := by
    rw [hd20d]; exact hst20
  obtain ⟨h21pc, h21stk, h21ee, _⟩ :=
    step_DUP2_shape_strong s20 s21 callFuel 0 none xV balV [UInt256.ofNat s16.executionEnv.source.val, xV] h20stk hst20
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s22, hst21, hrun⟩ := evmRunToCall_succ_step callFuel N s21 _ _ _ hd21d (by decide) (by decide) hrun
  have h21' : EVM.step (callFuel + 1) 0 (decode s21.executionEnv.code s21.pc) s21 = .ok s22 := by
    rw [hd21d]; exact hst21
  obtain ⟨h22pc, h22stk, h22ee, _⟩ :=
    step_LT_shape_strong s21 s22 callFuel 0 none balV xV [balV, UInt256.ofNat s16.executionEnv.source.val, xV] h21stk hst21
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s23, hst22, hrun⟩ := evmRunToCall_succ_step callFuel N s22 _ _ _ hd22d (by decide) (by decide) hrun
  have h22' : EVM.step (callFuel + 1) 0 (decode s22.executionEnv.code s22.pc) s22 = .ok s23 := by
    rw [hd22d]; exact hst22
  obtain ⟨h23pc, h23stk, h23ee, _⟩ :=
    step_PUSH_shape_strong s22 s23 callFuel 0 .PUSH2 (by decide) revertLbl 2 hst22
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s24, hst23, hrun⟩ := evmRunToCall_succ_step callFuel N s23 _ _ _ hd23d (by decide) (by decide) hrun
  have h23' : EVM.step (callFuel + 1) 0 (decode s23.executionEnv.code s23.pc) s23 = .ok s24 := by
    rw [hd23d]; exact hst23
  obtain ⟨h24pc, h24stk, h24ee, _⟩ :=
    step_JUMPI_shape_strong s23 s24 callFuel 0 none revertLbl (UInt256.lt balV xV)
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s25, hst24, hrun⟩ := evmRunToCall_succ_step callFuel N s24 _ _ _ hd24d (by decide) (by decide) hrun
  have h24' : EVM.step (callFuel + 1) 0 (decode s24.executionEnv.code s24.pc) s24 = .ok s25 := by
    rw [hd24d]; exact hst24
  obtain ⟨h25pc, h25stk, h25ee, _⟩ :=
    step_DUP3_shape_strong s24 s25 callFuel 0 none balV (UInt256.ofNat s16.executionEnv.source.val) xV [] h24stk hst24
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s26, hst25, hrun⟩ := evmRunToCall_succ_step callFuel N s25 _ _ _ hd25d (by decide) (by decide) hrun
  have h25' : EVM.step (callFuel + 1) 0 (decode s25.executionEnv.code s25.pc) s25 = .ok s26 := by
    rw [hd25d]; exact hst25
  obtain ⟨h26pc, h26stk, h26ee, _⟩ :=
    step_SWAP1_shape_strong s25 s26 callFuel 0 none xV balV [UInt256.ofNat s16.executionEnv.source.val, xV] h25stk hst25
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s27, hst26, hrun⟩ := evmRunToCall_succ_step callFuel N s26 _ _ _ hd26d (by decide) (by decide) hrun
  have h26' : EVM.step (callFuel + 1) 0 (decode s26.executionEnv.code s26.pc) s26 = .ok s27 := by
    rw [hd26d]; exact hst26
  obtain ⟨h27pc, h27stk, h27ee, _⟩ :=
    step_SUB_shape_strong s26 s27 callFuel 0 none balV xV [UInt256.ofNat s16.executionEnv.source.val, xV] h26stk hst26
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s28, hst27, hrun⟩ := evmRunToCall_succ_step callFuel N s27 _ _ _ hd27d (by decide) (by decide) hrun
  have h27' : EVM.step (callFuel + 1) 0 (decode s27.executionEnv.code s27.pc) s27 = .ok s28 := by
    rw [hd27d]; exact hst27
  obtain ⟨h28pc, h28stk, h28ee, _⟩ :=
    step_SWAP1_shape_strong s27 s28 callFuel 0 none (UInt256.sub balV xV) (UInt256.ofNat s16.executionEnv.source.val) [xV] h27stk hst27
  ...
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s29, hst28, hrun⟩ := evmRunToCall_succ_step callFuel N s28 _ _ _ hd28d (by decide) (by decide) hrun
  have h28' : EVM.step (callFuel + 1) 0 (decode s28.executionEnv.code s28.pc) s28 = .ok s29 := by
    rw [hd28d]; exact hst28
  obtain ⟨h29pc, h29stk, h29ee⟩ :=
    step_SSTORE_shape s28 s29 callFuel 0 none (UInt256.ofNat s16.executionEnv.source.val) (UInt256.sub balV xV) [xV] h28stk hst28
  ...
  -- s29.stack = [xV]
  have h29stk' : s29.stack = [xV] := h29stk
  -- ===== CALL-SETUP (PCs 61–71): peel to just before the external CALL =====
  -- 29: PUSH1 0 @ 61
  have hd29c : decode s29.executionEnv.code s29.pc = some (.Push .PUSH1, some (UInt256.ofNat 0, 1)) := by
    rw [hc29, hp29]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s30, hst29, hrun⟩ := evmRunToCall_succ_step callFuel N s29 _ _ _ hd29c (by decide) (by decide) hrun
  obtain ⟨h30pc, h30stk, h30ee, h30am⟩ := step_PUSH1_shape_strong s29 s30 callFuel 0 (UInt256.ofNat 0) hst29
  rw [h29stk'] at h30stk
  have hc30 : s30.executionEnv.code = bytecode := by rw [h30ee]; exact hc29
  have hp30 : s30.pc = UInt256.ofNat 63 := by rw [h30pc, hp29]; decide
  -- 30: PUSH1 0 @ 63
  have hd30c : decode s30.executionEnv.code s30.pc = some (.Push .PUSH1, some (UInt256.ofNat 0, 1)) := by
    rw [hc30, hp30]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s31, hst30, hrun⟩ := evmRunToCall_succ_step callFuel N s30 _ _ _ hd30c (by decide) (by decide) hrun
  obtain ⟨h31pc, h31stk, h31ee, h31am⟩ := step_PUSH1_shape_strong s30 s31 callFuel 0 (UInt256.ofNat 0) hst30
  rw [h30stk] at h31stk
  have hc31 : s31.executionEnv.code = bytecode := by rw [h31ee]; exact hc30
  have hp31 : s31.pc = UInt256.ofNat 65 := by rw [h31pc, hp30]; decide
  -- 31: PUSH1 0 @ 65
  have hd31c : decode s31.executionEnv.code s31.pc = some (.Push .PUSH1, some (UInt256.ofNat 0, 1)) := by
    rw [hc31, hp31]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s32, hst31, hrun⟩ := evmRunToCall_succ_step callFuel N s31 _ _ _ hd31c (by decide) (by decide) hrun
  obtain ⟨h32pc, h32stk, h32ee, h32am⟩ := step_PUSH1_shape_strong s31 s32 callFuel 0 (UInt256.ofNat 0) hst31
  rw [h31stk] at h32stk
  have hc32 : s32.executionEnv.code = bytecode := by rw [h32ee]; exact hc31
  have hp32 : s32.pc = UInt256.ofNat 67 := by rw [h32pc, hp31]; decide
  -- 32: PUSH1 0 @ 67
  have hd32c : decode s32.executionEnv.code s32.pc = some (.Push .PUSH1, some (UInt256.ofNat 0, 1)) := by
    rw [hc32, hp32]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s33, hst32, hrun⟩ := evmRunToCall_succ_step callFuel N s32 _ _ _ hd32c (by decide) (by decide) hrun
  obtain ⟨h33pc, h33stk, h33ee, h33am⟩ := step_PUSH1_shape_strong s32 s33 callFuel 0 (UInt256.ofNat 0) hst32
  rw [h32stk] at h33stk
  have hc33 : s33.executionEnv.code = bytecode := by rw [h33ee]; exact hc32
  have hp33 : s33.pc = UInt256.ofNat 69 := by rw [h33pc, hp32]; decide
  -- 33: DUP5 @ 69
  have hd33c : decode s33.executionEnv.code s33.pc = some (.DUP5, none) := by
    rw [hc33, hp33]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s34, hst33, hrun⟩ := evmRunToCall_succ_step callFuel N s33 _ _ _ hd33c (by decide) (by decide) hrun
  obtain ⟨h34pc, h34stk, h34ee, h34am⟩ :=
    step_DUP5_shape_strong s33 s34 callFuel 0 none (UInt256.ofNat 0) (UInt256.ofNat 0)
      (UInt256.ofNat 0) (UInt256.ofNat 0) xV [] h33stk hst33
  have hc34 : s34.executionEnv.code = bytecode := by rw [h34ee]; exact hc33
  have hp34 : s34.pc = UInt256.ofNat 70 := by rw [h34pc, hp33]; decide
  -- 34: CALLER @ 70
  have hd34c : decode s34.executionEnv.code s34.pc = some (.CALLER, none) := by
    rw [hc34, hp34]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s35, hst34, hrun⟩ := evmRunToCall_succ_step callFuel N s34 _ _ _ hd34c (by decide) (by decide) hrun
  obtain ⟨h35pc, h35stk, h35ee, h35am⟩ := step_CALLER_value s34 s35 callFuel 0 none hst34
  have hc35 : s35.executionEnv.code = bytecode := by rw [h35ee]; exact hc34
  have hp35 : s35.pc = UInt256.ofNat 71 := by rw [h35pc, hp34]; decide
  -- 35: GAS @ 71
  have hd35c : decode s35.executionEnv.code s35.pc = some (.GAS, none) := by
    rw [hc35, hp35]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  obtain ⟨s36, hst35, hrun⟩ := evmRunToCall_succ_step callFuel N s35 _ _ _ hd35c (by decide) (by decide) hrun
  obtain ⟨h36pc, ⟨g, h36stk⟩, h36ee, h36am⟩ := step_GAS_shape_strong s35 s36 callFuel 0 none hst35
  have hc36 : s36.executionEnv.code = bytecode := by rw [h36ee]; exact hc35
  have hp36 : s36.pc = UInt256.ofNat 72 := by rw [h36pc, hp35]; decide
  -- 36: CALL @ 72 — stop, s_pre = s36
  have hd36c : decode s36.executionEnv.code s36.pc = some (.CALL, none) := by
    rw [hc36, hp36]; native_decide
  obtain _ | N := N
  · simp [evmRunToCall] at hrun
  have hspre : s_pre = s36 := evmRunToCall_at_call callFuel N s36 _ _ _ hd36c (by decide) hrun
  -- the call-setup ops touch no storage, so the recorded balance is the post-SSTORE one
  have ham36 : s36.accountMap = s29.accountMap := by
    rw [h36am, h35am, h34am, h33am, h32am, h31am, h30am]
  rw [hspre, show recordedBalance s36.accountMap C (msgSender s0)
        = recordedBalance s29.accountMap C (msgSender s0) from by rw [ham36]]
  exact hrb29
